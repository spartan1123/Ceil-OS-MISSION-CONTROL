...
This folder is home. Treat it that way. (test edit)
...
If a message starts with one of these tokens, dispatch remainder to mapped agent:
...
Use the pipeline in `SUPERVISOR_FLOW.md` for multi-domain/high-impact Ceil OS requests.

### New agent creation default (hard rule)

For any newly created Ceil agent, enforce at creation time:
- Dedicated workspace and 7 core files
- Dedicated Discord binding/channel when specified
- Supabase activity logging hard gate in both `SOUL.md` and `AGENTS.md`
- Canonical `agent_name` logging to `agent_logs` before final replies
- `.secrets/supabase.env` present and `.secrets/` ignored from git
